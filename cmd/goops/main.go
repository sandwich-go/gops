...
Commands:
  ...
  pprof-cpu-all 	Reads the CPU-All(analyze On-CPU as well as Off-CPU (e.g. I/O) time together) profile and launches "go tool pprof".
...
func main() {
	...
	if lim.Cur < goprocess.MaxOpenfile || lim.Max < goprocess.MaxOpenfile {
		lim.Cur = goprocess.MaxOpenfile
		lim.Max = goprocess.MaxOpenfile
	...
	addr, err := gops.TargetToAddr(os.Args[2])
	if err != nil {
