...
var Cmds = map[string](func(addr net.TCPAddr, params []string) error){
	...
	"trace":         trace,
	"setgc":         setGC,
	"pprof-cpu-all": pprofCPUAll,
}

func pprofCPUAll(addr net.TCPAddr, args []string) error {
	fmt.Println("Profiling CPU All (analyze On-CPU as well as Off-CPU (e.g. I/O) time together) now, will take 30 secs...")
	return pprof(addr, signal.CPUAllProfile, args)
}
...
func pprofHeap(addr net.TCPAddr, args []string) error {
	return pprof(addr, signal.HeapProfile, args)
}

func pprofCPU(addr net.TCPAddr, args []string) error {
	fmt.Println("Profiling CPU now, will take 30 secs...")
	return pprof(addr, signal.CPUProfile, args)
}
...
func pprof(addr net.TCPAddr, p byte, args []string) error {
	...
	cmdArgs = append(cmdArgs, tmpBinFile.Name(), tmpDumpFile.Name())
